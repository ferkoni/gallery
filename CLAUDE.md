...
Auth flow: `POST /api/v1/users/login` returns a JWT token. Frontend stores it in `sessionStorage` and sends it as `Authorization: Bearer <token>` on subsequent requests.
